        ...
        <div className="mx-auto max-w-[1440px] px-6 md:px-10">
          <div className="max-w-2xl">
            <a
              ...
              className="inline-flex items-center justify-center h-12 sm:h-14 px-8 sm:px-10 bg-ink text-paper text-sm tracking-wider hover:bg-ink/90 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200 w-full sm:w-auto"
            >
              네이버에서 예약·정보 보기 →
            </a>
            <p className="mt-4 text-xs sm:text-sm text-stone leading-relaxed">
        ...
        <div className="mx-auto max-w-[1440px] px-6 md:px-10">
          ...
          <div className="mt-10 grid gap-px bg-black/10 sm:grid-cols-2 lg:grid-cols-3 border border-black/10">
            {SERVICE_CATEGORIES.map((cat) => (
              <div key={cat.title} className="bg-paper p-5 sm:p-7 md:p-8">
        ...
        <div className="mx-auto max-w-[1440px] px-6 md:px-10">
          <div className="grid gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
            {["inside1", "inside2", "inside3", "inside4"].map((name) => (
              <div key={name} className="relative aspect-[4/3] sm:aspect-[3/4] overflow-hidden bg-cream">
                <Image
                  src={`/images/${name}.png`}
                  alt="메이원헤어 매장"
                  fill
